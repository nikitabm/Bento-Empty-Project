    ...
    */
    extension.WebDialog = function() {
        ...
        if (Cocoon.App.nativeAvailable) {
            ...
            iframe.style.cssText = "width:100%; height:100%;margin:0;padding:0;";
            var me = this;
        ...
        */
        show: function(url, callback) {
            ...
            if (Cocoon.App.nativeAvailable) {
                ...
                this.iframe.src = url;
                var wrapper = document.getElementById('cocoonjswrapper');
                if (!wrapper) {
                    wrapper = document.createElement('div');
                    wrapper.id = 'cocoonjswrapper';
    ...
    extension.SocialGamingService.prototype = {
        ...
        showAchievements : function(callback) {
            ...
            this.requestAchievements(function(achievements, error){
                ...
                // hack
                if (navigator.isCocoonJS) {
                    Cocoon.App.loadInTheWebView("highscore/achievements.html", {
                         success: function() {
                             Cocoon.App.showTheWebView();
                             Cocoon.App.forwardAsync(js);
                         },
                         error: function() {
                             console.log("Cannot show the Webview for some reason :/");
                             console.log(JSON.stringify(arguments));
                ...
                this.gapi.client.request({path: this.gapi.gamesAPI + "/leaderboards/" + leaderboardID + "/window/" + collection,
                    method: "GET", params:{timeSpan: timeSpan}, callback: function(response) {
                        ...
                        if (response && response.nextPageToken) {
                            ...
                            js += "nextPageToken('gapi', '" + response.nextPageToken + "');";
                        } else {
                            nextPage = null;
                        }
                        var myId = 0;
                        if (response && response.playerScore) {
                            ...
                            myId = response.playerScore.player.playerId;
                            var item = response.playerScore,
                                score = fromGPScoreToCocoonScore(item, leaderboardID);
                            score.imageURL+="?sz=50";
                            score.position = item.scoreRank;
                            js += 'addMyScore(' + JSON.stringify(score) + ');';
                        }
                        for (var i = 0; i< items.length; ++i) {
                            ...
                            score.me = myId === item.player.playerId;
                            scores.push(score);
                        }
                        js += "addScores(" + JSON.stringify(scores) + ");";
                        dialog.eval(js);

                        // hack
                        if (navigator.isCocoonJS) {
                            Cocoon.App.loadInTheWebView("highscore/index.html", {
                                 success: function() {
                                    Cocoon.App.showTheWebView();
                                    Cocoon.App.forwardAsync(js);
                                 },
...
Cocoon.define("Cocoon.Social" , function(extension){
        ...
        */
        getPermissions: function(callback) {
            this.api('me/permissions', function(response) {
                ...
                // find the publish actions permission!
    ...
    extension.SocialGamingServiceFacebook.prototype =  {
        ...
        preparePublishAction: function(callback) {
            ...
            } else if (this.currentPermissions.permission === 'publish_actions' && 
                this.currentPermissions.status === 'granted') {
                callback(true);
            } else {
        ...
        showLeaderboard : function(callback, params) {
            ...
            this.fb.api(me.fb._appId + "/scores", function(response) {
                ...
                // hack
                if (navigator.isCocoonJS) {
                    Cocoon.App.loadInTheWebView("highscore/indexfb.html", {
                         success: function() {
                            Cocoon.App.showTheWebView();
                            Cocoon.App.forwardAsync(js);
                         },
        ...
        showAchievements : function(callback) {
            ...
            this.requestAchievements(function(achievements, error){
                ...
                // hack
                if (navigator.isCocoonJS) {
                    Cocoon.App.loadInTheWebView("highscore/achievementsfb.html", {
                         success: function() {
                             Cocoon.App.showTheWebView();
                             Cocoon.App.forwardAsync(js);
                         },
